...
int main(int argc, char *argv[]) {
  ...
  add_signal_listener(&wm.cursor->events.motion, &wm.cursor_motion_listener,
                      cursor_motion);
  add_signal_listener(&wm.cursor->events.motion_absolute,
                      &wm.cursor_motion_absolute_listener,
                      cursor_motion_absolute);
  add_signal_listener(&wm.cursor->events.button, &wm.cursor_button_listener,
                      cursor_button);
  add_signal_listener(&wm.cursor->events.axis, &wm.cursor_axis_listener,
                      cursor_axis);
  add_signal_listener(&wm.cursor->events.frame, &wm.cursor_frame_listener,
                      cursor_frame);
  add_signal_listener(&wm.backend->events.new_input, &wm.new_input_listener,
                      new_input);
  wm.seat = wlr_seat_create(wm.display, "seat0");
  add_signal_listener(&wm.seat->events.request_set_cursor,
                      &wm.request_set_cursor_listener, request_set_cursor);
  add_signal_listener(&wm.seat->pointer_state.events.focus_change,
                      &wm.focus_change_listener, focus_change);
  add_signal_listener(&wm.seat->events.request_set_selection,
                      &wm.request_set_selection_listener,
  ...
  // center cursor here
  wlr_cursor_warp(wm.cursor, NULL, 1920/2, 1050/2);
